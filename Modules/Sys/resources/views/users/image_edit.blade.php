...
@section('content')
    {!! Form::model($user, [
        'route' => ['sys.users.imageUpdate', ['id' => $user->id, 'field' => $field]],
        'method' => 'patch',
    ]) !!}
    <div class="modal-body">
        <!-- Profile Image Field -->
        <div class="form-group col-sm-8 {{ validationClass($errors->has($field)) }}">
            {!! Form::label($field, __('sys::models/users.fields.' . $field) . ':') !!}
            <div class="clearfix"></div>
            {!! Form::hidden($field . '_pre', !empty($user->$field) ? $user->$field : null) !!}
            {!! Form::file($field, [
                'id' => 'filepond1',
                'class' => 'my-pond',
            ]) !!}
            <div class="image-help-text" id="basic-addon4"> {{ __('sys::models/users.optimal_image_size') }}</div>
            ...
            @if (old($field) && file_exists(storage_path('tmp/' . old($field))))
                <p class="temp-image m-1">
                    ...
                    {!! renderTmpImage(old($field), 200) !!}
                    {!! renderTmpImageDeleteIcon(old($field)) !!}
                </p>
            @elseif (!empty($user->$field))
                <p class="del-form-image m-1">
                    {!! Form::hidden($field, !empty($user->$field) ? $user->$field : null, [
                        'id' => 'filepond1-' . $field,
                    ]) !!}
                    {!! renderImage(USER_FILE_DIR_NAME, $user->$field, 200) !!}
                    {!! renderImageDeleteIcon(route('sys.users.removeImage', [$user->id]), $field) !!}
                </p>
            @endif
        </div>
    </div>
